...
class _CategoryTabState extends State<CategoryTab>
  ...
  void didChangeDependencies() {
    ...
    Provider.of<SettingsProvider>(context)
        .addListener(updateCategoiesAndSubCategories);
  }
  ...
  void dispose() {
    if (mounted) {
      Provider.of<SettingsProvider>(context, listen: false)
          .removeListener(updateCategoiesAndSubCategories);
    }
  ...
  Future<void> updateCategoiesAndSubCategories() async {
    ...
    for (Category cat in categories) {
      try {
        await getSubCategoryByCategoryId(
                cat.id!,
                Provider.of<SettingsProvider>(context, listen: false)
                    .languageCode)
            .then((value) => setState(() {
                  ...
                  print(
                      'VALUE OF SUBCATEGORIES of category ${cat.id} FOR CATPAGE: $value');
                }));
      } catch (e, s) {
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < cat.length; i++) {
      int categoryId = categories[i]
          .id!; // Get the subcategories from the map using the category id
